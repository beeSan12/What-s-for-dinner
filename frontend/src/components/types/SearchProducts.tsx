...
const SearchProducts: React.FC<Props> = ({
  ...
  ? results.slice(page * maxResults, (page + 1) * maxResults)
  : results
  ...
  <div style={minimalLayout ? undefined : styles.container}>
    ...
    <ul style={styles.list}>
      {sliceResults.map(p => (
        <li key={p._id} style={styles.item}>
          ...
          <em>Brand:</em> {p.brands} <br />
          <button onClick={() => onProductSelect(p)}>＋ Add</button>
        </li>
      ))}
    </ul>
    ...
    {maxResults && results.length > maxResults && (
      <div style={styles.paging}>
        ...
        <button
          onClick={() => updatePage(page + 1)}
          disabled={(page + 1) * maxResults >= results.length}
        >
          →
        </button>
      </div>
